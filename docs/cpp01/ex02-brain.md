...
- **ポインタ** は「**別の場所** にあるアドレス値」を保持する**独立した変数**。`int *p` は `int` のアドレスを入れる箱で、その箱自体がメモリ上に存在する
- **参照** は「**元の変数そのもの**」に付けた**別名 (あだ名)** で、新しい記憶領域は作らない (実装上はポインタで実現されることが多いが、概念上は「別名」)

要するに、ポインタは「**住所を書いたメモ**」、参照は「**変数自身に付けたシール**」と考えるとイメージが近いです。
...
「NULL 不可」「再代入不可」「初期化必須」「`*` が要らない」— これらは全部、参照が **「別名」** という概念から自然に導かれる結果です。**仕様の都合** ではなく **概念の必然** として理解すると腹落ちします。

| 制約 | 概念上の理由 |
|---|---|
| **NULL にできない** | 参照は「**ある変数の別名**」。「**何の別名でもない**」という状態が言語的に作れない (誰の別名なのか答えられない) |
| **後から別の変数に付け替え不可** | 「あだ名を別人に付け替える」という動作は現実世界にもない概念。`r = y;` と書くと「別名 r が指している人 (= 元の x) に y を代入」という意味になり、別名そのものは付け替わらない |
| **宣言時に初期化必須** | 誰の別名なのか決めずに別名を作ることはできない。これも「あだ名」概念から自然 |
| **アクセスに `*` が要らない** | 別名はもう本体扱い。「メモ用紙からたどる」というワンクッションが概念上不要。だからデリファレンス (`*`) という発想自体がない |

つまり「**参照 = 別名**」という 1 つの考え方を覚えておけば、4 つの制約が全部一発で説明できます。

### 参照とポインタ、どっちを使う？ — 実務判断基準

C++ では「**使えるなら参照、必要ならポインタ**」が基本方針です。

| シーン | 推奨 | 理由 |
|---|---|---|
| **関数の引数で「変更しない」 (read only)** | `const T &` | コピーコストなし、NULL 心配なし、`*` も不要で書きやすい |
| **関数の引数で「中を書き換える」(out parameter)** | `T &` | 同上。引数が NULL になることはあり得ない前提 |
| **「対象が無い」状態があり得る引数** | `T *` | NULL を許容したい場合はポインタしかない |
| **クラスのメンバで「絶対に存在する」もの (cpp01 ex03 Weapon)** | `T &` メンバ | コンストラクタで束縛したら寿命中ずっと有効、再代入不可で書き換えバグ防止 |
| **クラスのメンバで「後から差し替える / 無い時もある」もの** | `T *` メンバ | 再代入可能、`NULL` で「無い」を表現できる |
| **配列・コンテナを巡回する** | `T *` | ポインタ算術 (`++p`) ができる、参照ではできない |
| **動的確保 (`new`) の戻り値を保持** | `T *` | `new` はポインタを返すので必然 |

迷ったら「**NULL あり得る？ → ポインタ。後で差し替える？ → ポインタ。それ以外 → 参照**」で大体合います。
